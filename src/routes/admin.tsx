...
const ALLOWED_ACCEPT = "application/pdf,.pdf,image/png,image/jpeg,image/jpg,image/webp,.jpg,.jpeg,.png,.webp";
    ...
    if (bulkUploadMode && pdfs.length > 1) {
      setSlots((prev) => {
        ...
        if (isTargetEmpty) {
          ...
          updated[targetIndex] = {
            ...targetSlot,
            files: [...targetSlot.files, ...nonPdfs],
          };
        }
        ...
        for (let i = pdfIndex; i < pdfs.length; i++) {
          const f = pdfs[i];
          const extractedName = f.name.replace(/\.[^/.]+$/, "");
          newSlots.push({
